...
public:
  ...
  //double kappa = 4.395; // L3 update and development for anopheles
  double kappa; // L3 update and development for anopheles
  double min_kappa = 4.395;
  double max_kappa = 4.395;
  double r; // L3 update and development for anopheles
  double min_r = 0.055;
  double max_r = 0.055;
  int species = 1; // 0 for culex, 1 for anopheles
  ...
  double s_2 = 0.00275; // Proportion of L3 entering host that develop into adults
  double mu = 0.0104; // worm death rate
  double gamma = 0.1; // mf death rate
  double alpha; // mf birth rate per female worm
  double min_alpha = 0.58; // 0.58
  double max_alpha = 0.58; // 0.58
...
void Data::calculate_prevalence() {
  ...
  double current_wormprev = worm_pos == 0 ? 0 : worm_pos/pop_size * 100;
  worm_prev.push_back(current_wormprev);
  mf_prev.push_back(mean(mf_ind) * 100);
...
void Data::set_parameter(std::string parameter, double value) {
  ...
  if(parameter == "systematic_compliance") {
    systematic_compliance = value;
    return;
  }
